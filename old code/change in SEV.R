...
dat4 = dat3 %>% 
  ...
  drop_na(eez_iso) %>% 
  left_join(pop, by=c("eez_iso" = "iso3c")) %>% 
  mutate(your_country = if_else(is.na(your_country), 0, your_country),
         ...
         per_capita = other_country/pop/365)
...
##Load data
per_capita_supply <- read_csv("outputs/foreign_nut_supply.csv") %>%
...
# Read GENUS dataset
genus_orig <- readRDS("~/MPA_Nutrition/Data/genus_nutrient_supplies_w_fort_by_age_sex_2011.Rds")

# Read COSIMO-GENUS I3O matching key
cosimo_genus_iso_key <- readxl::read_excel("~/MPA_Nutrition/tables/TableS4_cosimo_genus_countries.xlsx", skip=1) %>% 
...
spade_scalars <- readRDS(file.path("~/MPA_Nutrition/Data/intake_distribution_age_sex_scalars.Rds")) %>% 
...
# Read fits
dists <- readRDS("~/MPA_Nutrition/Data/intake_distributions_for_all_cosimo_countries.Rds") %>% 
...
omega_N_raw_2019 <- openxlsx::read.xlsx('~/MPA_Nutrition/data/omega_RR_2019.xlsx')
red_meat_raw_2019 <- openxlsx::read.xlsx('~/MPA_Nutrition/data/meat_RR_2019.xlsx')
EAR_requirements <- openxlsx::read.xlsx('~/MPA_Nutrition/data/EAR_requirements_GBDgroups.xlsx')
...
# Read HDI/SDI key
sdi_hdi_key <- readRDS("~/MPA_Nutrition/Data/COSIMO_country_key_with_SDI_HDI_info.rds") %>% 
  select(iso3, sdi, sdi_group, hdi)

# Source helpher functions
source("~/MPA_Nutrition/code/RR_functions.R")
...
# Export
write.csv(sev_mn_final, "outputs/sevs_base_high_foreign_catch.csv", row.names=F)
...
# Export
write.csv(sev_omega_final, "outputs/sevs_base_high_foreign_catch_o3.csv", row.names=F)

sev_final = rbind(read_csv("Outputs/sevs_base_high_foreign_catch.csv"), 
                  sev_omega_final)


write.csv(sev_final, "outputs/sevs_base_high_foreign_catch_final.csv", row.names=F)

####################Calculate number of people###################### 
# Read data
sevs <- read_csv("Outputs/sevs_base_high_foreign_catch_final.csv") %>% # sevs_meat
  # Format sex
...
pop_orig <- readRDS("~/MPA_Nutrition/Data/UN_WPP_2019_pop_proj_by_cntry_agesex.Rds")
...
##Export data
write.csv(data, "Outputs/impacted_pop_SEV_foreign_catch.csv", row.names = F)