...
##1)Catch distribution
countries_ISO <- read.csv("data/countries_ISO.csv", encoding = "UTF-8") %>% 
  distinct(missing_countries, .keep_all = T)

catch_iso <- read.csv("outputs/catch_by_country_2018.csv", encoding = "UTF-8") %>% 
  left_join(countries_ISO, by=c("fishing_entity" = "missing_countries"))

  # Format data
  mpa_dta_sf <- world %>% 
    left_join(catch_iso, by=c("gu_a3"="iso3c"))
  
  # Spatialize tiny
  sdata_pt <- world_centers %>% 
    left_join(catch_iso, by=c("iso3"="iso3c")) %>% 
    # Reduce to ones with data
    filter(!is.na(tonnes_iso)) %>% 
    arrange(area_sqkm) %>% 
    # Reduce to small
    filter(area_sqkm<=2.5*10^4 & continent!="Europe")
  
  plot1 <- ggplot(mpa_dta_sf) +
    geom_sf(mapping=aes(fill=tonnes_iso), lwd=0.1) +
    # Plot small places
    geom_sf(data=sdata_pt, mapping=aes(fill=tonnes_iso), shape=21, size=3, stroke=0.3) +
    # Plot French Guiana
    geom_sf(data=fguiana, lwd=0.1, color="grey30", fill="grey80") +
    # Crop out Antarctica
    coord_sf(y=c(-55, NA)) +
    # Legend and labels
    scale_fill_gradient2(name="Catch\n(million tonnes)",
                         #breaks=seq(0, 2.5, 0.5), labels=c("0", "0.5", "1.0", "1.5", "2.0", ">= 2.5"),
                         low="navy", high="darkred", mid="white", midpoint=0) +
    guides(fill = guide_colorbar(ticks.colour = "black", frame.colour = "black", barwidth = 0.6, barheight = 3)) +
    labs(title = "A - Industrial bottom-trawl catch")+
    # Theme
    theme_bw() + base_theme +
...
ggsave(filename = "Figures/catch_dist.jpeg", 
       plot = plot1,
       height = 4.5, 
       width = 10)

##2)Catch proportion distribution
# Format data
mpa_dta_sf <- world %>% 
  left_join(catch_iso, by=c("gu_a3"="iso3c"))
...
sdata_pt <- world_centers %>% 
  left_join(catch_iso, by=c("iso3"="iso3c")) %>% 
  # Reduce to ones with data
  filter(!is.na(perc_iso_tot)) %>% 
  arrange(area_sqkm) %>% 
  # Reduce to small
  filter(area_sqkm<=2.5*10^4 & continent!="Europe")

plot2 <- ggplot(mpa_dta_sf) +
  geom_sf(mapping=aes(fill=perc_iso_tot), lwd=0.1) +
  # Plot small places
  geom_sf(data=sdata_pt, mapping=aes(fill=perc_iso_tot), shape=21, size=3, stroke=0.3) +
  # Plot French Guiana
  geom_sf(data=fguiana, lwd=0.1, color="grey30", fill="grey80") +
  # Crop out Antarctica
  coord_sf(y=c(-55, NA)) +
  ...
  scale_fill_gradient2(name="Catch\n(%)",
                       ...
                       low="navy", high="darkred", mid="white", midpoint=0) +
  guides(fill = guide_colorbar(ticks.colour = "black", frame.colour = "black", barwidth = 0.6, barheight = 3)) +
  labs(title = "B - Percent catch from industrial bottom-trawl")+
  # Theme
  theme_bw() + base_theme +
  theme(legend.position=c(0.04,0.327),
        axis.text = element_blank(),
        axis.title=element_blank(), 
        legend.background = element_rect(fill=alpha('blue', 0)),
        legend.title = element_text(size = 10),
...
#############################Nutrient supply - ASF
nut_supply_GND <- read_csv("outputs/nut_supply_GND.csv")
...
ggsave(g, filename = "Figures/Nutrient supply relative to ASF.pdf", 
       width=10, height=6.2, units="in", dpi=600, device=cairo_pdf)

ggsave(g, filename = "Figures/Nutrient supply relative to ASF.jpeg", 
       height = 6.2, 
       width = 10)
...
ggsave(g, filename = "Figures/Nutrient supply relative to all other foods.pdf", 
       width=10, height=6.2, units="in", dpi=600, device=cairo_pdf)

ggsave(g, filename = "Figures/Nutrient supply relative to all other foods.jpeg", 
       height = 6.2, 
       width = 10)
...
ggsave(g, filename = "Figures/Nutrient supply relative to fish.pdf", 
       width=10, height=6.2, units="in", dpi=600, device=cairo_pdf)

ggsave(g, filename = "Figures/Nutrient supply relative to fish.jpeg", 
       height = 6.2, 
       width = 10)
