...
dat_total = data %>% 
  group_by(year) %>% 
  summarise(tonnes_tot = sum(tonnes, na.rm = T)/1000000)

dat_trawl = data %>% 
  filter(gear == "bottom trawl",
         sector == "Industrial") %>% 
  group_by(year) %>% 
  summarise(tonnes = sum(tonnes, na.rm = T)/1000000) %>% 
  left_join(dat_total) %>% 
  mutate(perc_trawl = 100*tonnes/tonnes_tot)

##Tonnes of catch by end use
dat_enduse = data %>% 
  filter(gear == "bottom trawl",
         sector == "Industrial") %>% 
  group_by(year, end_use) %>% 
  summarise(tonnes_end = sum(tonnes, na.rm = T)/1000000) %>% 
  left_join(dat_trawl) %>% 
  mutate(perc_enduse = 100*tonnes_end/tonnes)

##How many countries?
dat_total_iso = data %>% 
  group_by(year, fishing_entity) %>% 
  summarise(tonnes_tot_iso = sum(tonnes, na.rm = T)/1000000) %>% 
  filter(year == 2018)
...
dat_iso = data %>% 
  filter(gear == "bottom trawl",
         sector == "Industrial") %>% 
  group_by(year, fishing_entity) %>% 
  summarise(tonnes_iso = sum(tonnes, na.rm = T)/1000000) %>% 
  filter(year == 2018) %>% 
  left_join(dat_trawl) %>%
  left_join(dat_total_iso) %>% 
  mutate(perc_iso = 100*tonnes_iso/tonnes,
         perc_iso_tot = 100*tonnes_iso/tonnes_tot_iso)

write.csv(dat_iso, "outputs/catch_by_country_2018.csv", row.names = F)
